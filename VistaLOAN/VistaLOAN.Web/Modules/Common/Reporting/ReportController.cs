...
namespace VistaLOAN
{
    ...
    public class ReportController : Controller
    {
        ...
        public ActionResult GetLoanApplicationByEmployee(string empId)
        {
            if (!String.IsNullOrEmpty(empId))
            {
                ...
                {
                    var cc = PrmEmploymentInfoRow.Fields;
                    var employeeId = connection
                                    .First<PrmEmploymentInfoRow>(e => e.SelectTableFields()
                                                            .Select(cc.Id)
                                                            .Where(cc.EmpId == empId))?.Id;

                    var ap = LaLoanApplicationRow.Fields;
                    items = connection
                        .List<LaLoanApplicationRow>(a => a.SelectTableFields()
                                                        .Select(ap.Id)
                                                        .Select(ap.LoanNo)
                                                        .Where(ap.EmployeeId == (employeeId ?? 0)));
                    if (items != null)
                    {
                        return Json(new SelectList(items, "Id", "LoanNo"), JsonRequestBehavior.AllowGet);
                    }
                }
            }
